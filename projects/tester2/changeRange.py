...
def runIVSweeps_softwareAutoRange(*lstIVSweep : IVSweep, CSV_name):
    ...
    session = nidcpower.Session(resource_name=resources, reset=True,  options = {'range_check': False, 'cache': False, 'simulate': False, 'driver_setup': {'Model': '4135', 'BoardType': 'PXIe', }, })
    ...
    # dump vSteps
        for i in range(numOfSteps):
            ...
            session.wait_for_event(nidcpower.Event.SOURCE_COMPLETE, timeout=timeout)
            tos = time.time()
            check_compliance(resources, session, current_ranges)
            toe = time.time()
            # print(f"time elapse is {toe-tos}")
            time_count += (toe-tos)
            

            
            measurements = session.channels["SMU2/0"].measure_multiple()[0]
            df_meas_list.append(measurements.current)
            
    # for i in df_meas_list:
    #     print(i)   
    session.close()
    print(time_count)
...
def _test():
    ...
    #     E5250A_Simple.InputPort.SMU6: 8, #Bulk
    ...
    vtlin = IVSweep(ChnVoltSweep('G', 'SMU2/0', V_start=-0.05, V_stop=1.32, V_step=0.01, I_compl=1e-3, remote_sense=False),
                [ChnVoltBias('AF', 'SMU1/0', 0.1, I_compl=1e-3, remote_sense=True),
                ChnVoltBias('Guard', 'SMU4/0', 0.1, I_compl=1e-3),
                ChnVoltBias('S', 'SMU3/0', 0, I_compl=1e-3),
                ChnVoltBias('B', 'SMU5/0', 0, I_compl=1e-3),
    ...
    vtlin2 = IVSweep(ChnVoltSweep('G', 'SMU3/0', V_start=-0.5, V_stop=3.3, V_step=0.02, I_compl=1e-3, remote_sense=False), #5
                [ChnVoltBias('D', 'SMU2/0', 0.1, I_compl=1e-3, remote_sense=False), # 6
                ChnVoltBias('S', 'SMU1/0', 0, I_compl=1e-3), #7
                # ChnVoltBias('B', 'SMU4/9', 0, I_compl=1e-3),#8
    ...
    start = time.time()
    runIVSweeps_softwareAutoRange(vtlin2, CSV_name=demo)
    stop = time.time()
    print(stop - start)
